...
case class Cache( entries: Set[CacheEntry] ) {
  @transient lazy val pentries = entries.par
...
object ProviderFilesInformation {
  def createFromFiles( files: Traversable[File] ): FailureValidation[ProviderFilesInformation] = {
    def buildProviderFileInformationOrFailure( f: File ) =
      convertExceptions( f.getName ) { List( ProviderFileInformation( f.getPath, fileChecksum( f ) ) ).success }
  ...
  def createFromParameters( p: JartenderCacheParameters ): FailureValidation[ProviderFilesInformation] =
