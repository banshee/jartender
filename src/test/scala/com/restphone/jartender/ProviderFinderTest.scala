...
class ProviderFinderTest extends FunSuite with ShouldMatchers {
  ...
  def buildSampleTrait = {
    ...
    xs should be( 'success )
    listToStreamOfLists( xs.toOption.get.elements )
  }
  ...
  def buildJartenderSample = {
    val buildJartenderBase = {
      ...
      val xs = DependencyAnalyser.buildItemsFromClassName( name )
      xs should be( 'success )
      val result = xs.toOption.get.elements
  ...
  test( "can analyze a subclass" ) {
    ...
    val x = classOf[com.restphone.jartender.JartenderSample]
    val tr = Thread.currentThread().getContextClassLoader().getResourceAsStream( name.s + ".class" )
    val xs = DependencyAnalyser.buildItemsFromClassName( name )
    xs should be( 'success )
    val elements = xs.toOption.get.elements
    elements should not be ( 'empty )

    val methodInSubclass = elements collectFirst { case ProvidesMethod( _, _, JavaIdentifier( "methodInSubclass" ), _, _, _ ) => true }
